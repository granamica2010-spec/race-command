...
function forecast() {
  const plan = Array.isArray(state?.weatherForecast) && state.weatherForecast.length ? state.weatherForecast : [{offsetMinutes:0,...state.weather}];
  return plan.slice(0,4).map(item=>`<div title="${escapeHtml(item.label||item.name||'Meteo')}"><span>${item.offsetMinutes?`+${item.offsetMinutes}m`:'ORA'}</span><b>${item.icon||'☀️'}</b><span>${item.rain ?? 0}%</span></div>`).join('');
}
function trackSvg() {
