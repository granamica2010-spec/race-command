...
function tyreButtons(keys = ['S', 'M', 'H'], disabled = false) {
  return keys.map(k => `<button class="tyreBtn ${selectedTyre === k ? 'active' : ''}" data-tyre="${k}" ${disabled ? 'disabled' : ''}><span class="ring" style="border-color:${colors[k]}">${k}</span><b>${names[k]}</b></button>`).join('');
}
...
function renderLobby() {
  ...
  app.innerHTML = `<div class="app"><div class="topbar"><div><div class="brand"><b>RACE</b> COMMAND</div><h2 style="margin-top:3px">Lobby GP</h2></div><div class="badges"><span class="badge">STANZA ${state.code}</span><span class="badge">${humans} UMANI + ${bots} BOT = 6</span><button class="btn ghost" id="leave">ESCI</button></div></div><div class="lobbyGrid"><section class="panel"><div class="ey">Codice da condividere</div><div class="roomCode">${state.code}</div><p class="subtitle">La griglia è sempre da 6: ogni nuovo giocatore umano sostituisce automaticamente un bot.</p><div class="lobbyBtns"><button class="btn" id="share">CONDIVIDI INVITO</button></div><div class="players">${state.players.map(p => `<div class="playerRow"><i class="dot ${p.connected ? 'online' : ''}" style="background:${p.color}"></i><div><div class="pname">${escapeHtml(p.name)} ${p.id === state.hostId ? '👑' : ''}</div><div class="small">${p.isBot ? '🤖 BOT · ' : p.connected ? '🟢 ONLINE · ' : '⚪ OFFLINE · '}${names[p.tyre]}</div></div><div class="rowActions"><span class="ready ${p.ready ? 'on' : ''}">${p.isBot ? 'AUTO' : p.ready ? 'READY' : 'WAIT'}</span></div></div>`).join('')}</div></section><section class="panel"><div class="ey">Gomma di partenza</div><div class="tyres" id="tyres">${tyreButtons(['S', 'M', 'H'], me.ready)}</div>${me.ready ? '<div class="small" style="margin-top:7px">Annulla READY per cambiare mescola.</div>' : ''}<button class="btn ${me.ready ? '' : 'green'}" id="readyBtn" style="width:100%;margin-top:10px">${me.ready ? 'ANNULLA READY' : 'READY'}</button>${isHost ? `<div class="settingsCard"><div class="ey">Durata gara</div><select id="laps"><option value="3" ${state.maxLaps === 3 ? 'selected' : ''}>3 giri · rapidissima</option><option value="5" ${state.maxLaps === 5 ? 'selected' : ''}>5 giri · standard</option><option value="8" ${state.maxLaps === 8 ? 'selected' : ''}>8 giri · lunga</option><option value="10" ${state.maxLaps === 10 ? 'selected' : ''}>10 giri · GP</option></select></div><div class="settingsCard"><div class="ey">Griglia automatica</div><b>${humans} giocator${humans===1?'e':'i'} + ${bots} bot</b><div class="small" style="margin-top:4px">I bot vengono gestiti automaticamente dal server.</div></div><div class="lobbyBtns"><button class="btn primary" id="start" ${!humansReady ? 'disabled' : ''}>START RACE · 6 PILOTI</button></div>` : `<div class="settingsCard"><div class="ey">Durata gara</div><b>${state.maxLaps} giri</b></div>`}</section></div></div>`;
  document.getElementById('leave').onclick = leaveRoom;
  document.getElementById('share').onclick = shareInvite;
  document.querySelectorAll('[data-tyre]').forEach(b => b.onclick = () => { if (!me.ready) { selectedTyre = b.dataset.tyre; renderLobby(); } });
...
function duelOptions(role) {
  return role === 'attack'
    ? [['attack', 'ATTACK', '+2 · -8% gomma'], ['aggressive', 'AGGRESSIVE', '+4 · -16% · rischio contatto'], ['ersAttack', 'ERS ATTACK', '+3 · costa 15% ERS'], ['hold', 'HOLD POSITION', 'rinuncia · +8% ERS']]
    : [['defend', 'DEFEND', '+2 · -8% gomma'], ['hardDefend', 'HARD DEFEND', '+4 · -16% · rischio contatto'], ['ersDef', 'ERS DEFENCE', '+3 · costa 15% ERS'], ['noFight', "DON'T FIGHT", 'lascia passare · zero consumo']];
}
function renderBattle() {
  const d = state.duel, me = state.me, involved = d && [d.attackerId, d.defenderId].includes(me.id), role = me.id === d?.attackerId ? 'attack' : 'defend', opp = me.id === d?.attackerId ? d.defenderName : d?.attackerName, locked = !!d?.myChoice;
  app.innerHTML = `<div class="app battlePage"><div class="topbar"><div><div class="brand"><b>RACE</b> COMMAND · ${state.code}</div><h2 style="margin-top:3px">BATTLE FOR POSITION</h2></div><div class="badges"><span class="badge">${state.weather.icon} ${state.weather.name}</span><span class="badge">${d?.locked || 0} / 2 LOCKED</span></div></div><section class="panel battle">${!involved ? `<div class="waiting"><div class="battleIcon">⚔️</div><div class="screenTitle" style="font-size:30px">Duello in corso</div><p class="subtitle">${escapeHtml(d?.attackerName || '')} sta attaccando ${escapeHtml(d?.defenderName || '')}. Il prossimo turno partirà appena il duello è risolto.</p><div class="spectatorBattle"><b>${escapeHtml(d?.attackerName || '')}</b><span>VS</span><b>${escapeHtml(d?.defenderName || '')}</b></div></div>` : `<div class="battleTop"><div><div class="ey">${role === 'attack' ? 'Stai attaccando' : 'Stai difendendo'}</div><h2>YOU vs ${escapeHtml(opp)}</h2></div><span class="role">${role === 'attack' ? 'ATTACCANTE' : 'DIFENSORE'}</span></div><div class="arena"><div class="row"><b>YOU</b><b>${escapeHtml(opp)}</b></div><div class="lane"><div class="battleCar you" id="battleYou">YOU</div><div class="battleCar rival" id="battleRival">R</div></div><div class="reveal"><div class="choice" id="myReveal">${duelResult ? duelLabels[role === 'attack' ? duelResult.attackerChoice : duelResult.defenderChoice] : (locked ? 'LOCKED 🔒' : '?')}</div><b>VS</b><div class="choice" id="oppReveal">${duelResult ? duelLabels[role === 'attack' ? duelResult.defenderChoice : duelResult.attackerChoice] : '?'}</div></div><div class="big" style="text-align:center;margin-top:10px" id="duelScore">${duelResult ? (role === 'attack' ? `${duelResult.attackerScore} — ${duelResult.defenderScore}` : `${duelResult.defenderScore} — ${duelResult.attackerScore}`) : 'Scegli la mossa'}</div></div>${state.phase === 'duel' && !locked ? `<div class="duelGrid">${duelOptions(role).map(([k, t, s]) => `<button class="duelBtn ${selectedDuel === k ? 'active' : ''}" data-duel="${k}">${t}<small>${s}</small></button>`).join('')}</div><button class="btn primary" id="duelLock" style="width:100%;margin-top:10px" ${!selectedDuel ? 'disabled' : ''}>LOCK IN</button>` : `<div class="waiting compact"><b>${state.phase === 'duel_result' ? 'REVEAL!' : 'Scelta bloccata. Attendo l’avversario…'}</b></div>`}`}</section></div>`;
  if (involved && state.phase === 'duel' && !locked) {
    document.querySelectorAll('[data-duel]').forEach(b => b.onclick = () => { selectedDuel = b.dataset.duel; renderBattle(); });
